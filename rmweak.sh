...
# CAUTION!!! CAUTION!!! The following "tail -r" must be replaced by "tac" in GNU/Linux
tail -r $result | grep TS -A 6 | sed '/^[^{T]*$/d' | awk -F "'" -v limit="$ts_limit" '/^'\''TS/{ts=$4;next}{if(ts<limit){print $2;ts=limit+1}}' > $rmlist
